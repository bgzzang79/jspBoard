...
public class StudentDao implements StudentDaoInf {
	...
	/**
	 * Method : selectAllStudent 최초작성일 : 2018. 7. 9. 작성자 : PC09 변경이력 :
	 * 
	 * @return Method 설명 : 전체 학생 정보를 조회
	 */
	@Override
	public List<StudentVo> selectAllStudent() {
		SqlSession session = sqlSessionFactory.openSession();
		List<StudentVo> studentList = session
				.selectList("student.selectAllStudent");
		session.close();
	...
	/**
	 * Method : getStudent 최초작성일 : 2018. 7. 10. 작성자 : PC09 변경이력 :
	 * 
	 * @param id
	...
	/**
	 * Method : getStudentVo 최초작성일 : 2018. 7. 10. 작성자 : PC09 변경이력 :
	 * 
	 * @param studentVo
	 * @return Method 설명 : 학생 정보 조회
	 */
	...
	public List<StudentVo> getStudentPageList(Map<String, Integer> map) {
		SqlSession session = sqlSessionFactory.openSession();
		List<StudentVo> studentList = session.selectList(
				"student.getStudentPageList", map);
	...
	public StudentVo getStudentId(String std_id) {
		SqlSession session = sqlSessionFactory.openSession();
		StudentVo studentVo = session.selectOne("student.getStudentId", std_id);
		session.close();
	...
	// @Override
	// public int loginChk(StudentVo studentVo) {
	// SqlSession session = sqlSessionFactory.openSession();
	// int cnt = session.selectOne("student.loginChk",studentVo);
	// session.close();
	//
	// return cnt;
	// }
