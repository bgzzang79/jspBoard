...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//form-data; name="uploadFile"; filename="rabbit.jpg"
		String contentDisposition = uploadFilePart.getHeader("Content-Disposition");		
		logger.debug("contentDisposition : "+contentDisposition);
		
		String filename = new FileUtil().getFileName(contentDisposition);
		logger.debug("filename : "+filename);
		
		//파일 저장 경로 설정
		//실무 : 별도 storage
		//개발환경 : 임의 영역(서버 변경ㅇ시, 재기동시 업로드 파일 삭제됨)
		...
		// doPost 메서드 안에서 application(servletContext)
